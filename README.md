...
3. Start configured Sail services.
4. Link the directory in Herd when `runtime=herd`.
5. Secure the Herd site and isolate its PHP version when configured.
6. Generate `APP_KEY` when the project has an empty key.

It does not run migrations, create or drop databases, rewrite PHPUnit configuration, rewrite `compose.yaml`, or inject test-runner options.
